...
module.exports = helpandUsage = (req, options) => {
    ...
    const now_years2 = moment().year();
    const write_powered_years = now_years2 === 2022 ? now_years2 : `2022 - ${now_years2}`;
...
Program information:
- Running on     : ${process.platform === 'linux'?'"Linux/Ubuntu"':'"Unknown"'}
- platform       : "${process.platform}"
- arch           : "${process.arch}"
- Root           : "${process.cwd()}"
- Puppeteer      : ${process.platform === 'linux'?'"google-chrome-stuble"':'"Unknown"'}
...
Media screen size is avaiable for "desktop", "handphone", and "custom". But custom requires a viewport parameter for screen size.
...
Custom         :   ${req.protocol}://${req.hostname}/api/webscreen?url=${exam_url}&mediatype=custom&viewport=1080x720&responsetype=json
